    ...
    for (const lesson of unit.lessons) {
      ...
      for (const slide of lesson.slides) {
        ...
        try {
          ...
          // If it does, select it directly instead of creating a new blank shell.
          // Without this check, re-running would create duplicate blank slides.
          const slideMatchKeysList = slideMatchKeys(slide.slide_name);
          let existingSlideItem = null;
          for (const key of slideMatchKeysList) {
            const candidate = page.locator('mat-list-option').filter({ hasText: key }).first();
            if (await candidate.count().catch(() => 0) && await candidate.isVisible().catch(() => false)) {
              // Click the candidate to load its type into the right panel,
              // then verify the slide type matches before treating as existing.
              await clearOverlays(page);
          ...
          // SET SLIDE TYPE -- exact backup sequence, no extra reads or waits
          await clearOverlays(page);
          const slideTypeSelect = page.locator('mat-select[formcontrolname="slideType"]').first();
          await slideTypeSelect.waitFor({ state: 'visible', timeout: 10000 });
